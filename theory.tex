...
\section{Relationship between the VNI and the redundancy of nodes}
...
\begin{equation}
    \varepsilon\text{-ENN}\overset{\Delta}{=}max(\{t\in\mathbb{N}: \lambda_t\geq\varepsilon\lambda_1\}).
    \label{eENN_def}
\end{equation}
...
$(t-1)$ $\varepsilon\lambda_1$ and $(N-1)$ zeros. Insert these eigenvalues into \eqref{rv_eigen}, we have
\begin{equation}
    \begin{aligned}
        \lambda_1+(t-1)\varepsilon\lambda_1 &= N \\
        \lambda_1^2+(t-1)(\varepsilon\lambda_1)^2 &= N^2R_{sq}.
    \end{aligned}
    \label{effective_eigen}
\end{equation}
Inserting the first equation in \eqref{effective_eigen} into the second one, we can get
\begin{equation}
    [1+(t-1)\varepsilon]^2=\Big(\frac{N}{\lambda_1}\Big)=\frac{1+(t-1)\varepsilon^2}{R_{sq}},
    \label{quad_eenn}
\end{equation}
...
After the differentiation of the \eqref{quad_eenn}, the derivative of $R_{sq}$ with respect to $t$ is
\begin{equation}
    \begin{aligned}
    \frac{dR_{sq}}{dt}&=
    \frac{[1+(t-1)\varepsilon]^2\cdot\varepsilon^2
    -[1+(t-1)\varepsilon^2]\cdot 2\varepsilon[1+(t-1)\varepsilon]}
    {[1+(t-1)\varepsilon]^4}\\
    &=\varepsilon
    \frac{[1+(t-1)\varepsilon]\cdot\varepsilon
    -2[1+(t-1)\varepsilon^2]}
    {[1+(t-1)\varepsilon]^3}\\
    &<0.
    \end{aligned}
    \label{negative_diff}
...
From \eqref{negative_diff}, we can derive the Theorem \ref{oppo}.
...
Also, insert the $R_{sq}=1$ condition into \eqref{quad_eenn}, then the solution of $t$ becomes $1$ 
(no matter the value of $\varepsilon$). Therefore, the Theorem \ref{collapse} can be proved.

\begin{theorem}[Network collapsing]
    \label{collapse}
    For any theshold $\varepsilon$,
    \\the $\varepsilon$-effective number of nodes becomes only 1 when the VNI $R_{sq}$ is 1.
\end{theorem}
